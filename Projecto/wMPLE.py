import numpy as np
def wMPLE(analyte, parameters, reps):
...
#initialize parameters
    ...
    #format data for simultaneous fitting
    """Dilution becomes nx2, then a column vector"""
    dilution = log(analyte(:,1)) 
    ...
    dilution = dilution( : )                 
    an1 = analyte(:,reps)                    #sample reps
    pixint = an1( : )                        #make one column vector from all columns
...
[nobs] = length(analyte);
    err = nobs/2*log(2*pi*sigma^2)+nobs*sum(constant.*weights.*(pixintpred-pixint).^2)/(2*sigma^2)...
